...
function ChatPage() {
  ...
  const handleSendMessage = (e) => {
    e.preventDefault();
    if (stompClient && connected && dialogId && message.trim() !== '' && currentUser) {
  ...
  useEffect(() => {
    const fetchDialogData = async () => {
        if (me && currentUser) {
            let dialog;
            if (!dialogId) {
                dialog = await sendGetRequest(`${SERVER}/messages/dialog?user1Id=${me.id}&user2Id=${currentUser.id}`);
                setDialogId(dialog.data);
            }
        }
    }
  ...
  useEffect(() => {
    ...
    if (stompClient && connected && currentUser && me && dialogId) {
      
      const uniqueTopic = `/topic/chat/${dialogId}`;
    ...
    fetchData();
}, [stompClient, connected, currentUser, me, dialogId]);
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (me && currentUser && dialogId) {
        const fetchedMessages = await getAllMessagesBetweenUsers(dialogId, 0, pageSize);
        setMessages(fetchedMessages);
      }
    };

    fetchMessages();
  }, [me, currentUser, dialogId]);


  async function getAllMessagesBetweenUsers(dialogId, page, size) {
    const response = await sendGetRequest(`${SERVER}/messages/dialog/getAll?dialogId=${dialogId}&page=${page}&size=${size}`);
    ...
    const handleScroll = async () => {
      if (messagesContainerRef.current && me && currentUser) {
        ...
        if (scrollPosition <= bottomThreshold) {
          const newMessages = await getAllMessagesBetweenUsers(dialogId, nextPageNumber, pageSize);
  ...
  }, [me, currentUser, nextPageNumber, pageSize, dialogId, messagesContainerRef]); 
